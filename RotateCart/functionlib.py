...
# ---------------------------------------------------------------------------- #
def RotateCart(Itab, angle_deg, x0, y0):  # fonction Rotation en coordonnées Cartésiennes autour du centre de rotation (x0, y0)
    size = np.shape(Itab)  # tuple size (Ny Nx)
    Nx = size[1]  # Nx = cols, Ny = rows
    Ny = size[0]  # Nx = cols, Ny = rows
    ...
    for y in range(Ny):  # range de 0 a Ny-1
        for x in range(Nx):  # range de 0 a Nx-1
            xr = int((x-x0)*cos(angle_rad) - (y-y0)*sin(angle_rad) + x0)
            yr = int((x-x0)*sin(angle_rad) + (y-y0)*cos(angle_rad) + y0)
            if 0<=xr<Nx and 0<=yr<Ny :
                Jtab[y][x] = Itab[yr][xr]
